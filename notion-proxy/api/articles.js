// api/articles.js — Proxy Notion per Markettisti Anonimi
...
async function blocksToHtml(pageId, token) {
...
function parseFonti(src) {
  ...
  return (src.match(urlRe) || []).slice(0,4).map((url,i) => {
    try { return { nome: new URL(url).hostname.replace('www.',''), url }; }
    catch { return { nome: `Fonte ${i+1}`, url }; }
  });
}
...
export default async function handler(req, res) {
  ...
  if (!token) {
    res.status(500).json({ error: 'NOTION_TOKEN non configurato' });
    return;
  }
  ...
  try {
    let pages = [], cursor;
    do {
      const body = { page_size: 100, sorts: [{ property: 'Data', direction: 'descending' }], ...(cursor && { start_cursor: cursor }) };
      const r = await fetch(`https://api.notion.com/v1/databases/${DATABASE_ID}/query`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Notion-Version': '2022-06-28', 'Content-Type': 'application/json' },
        body: JSON.stringify(body)
      });
      if (!r.ok) { const e = await r.json(); res.status(r.status).json(e); return; }
    ...
    const articles = await Promise.all(pages.map(async page => {
      ...
      if (!contenuto.trim()) {
        contenuto = await blocksToHtml(page.id, token);
      } else {
